...
-- Get the current buffer filename with the filetype icon supplied
-- by devicons.
...
function M:get_current_filetype_icon()
    ...
    local file_name, file_extension = vim.fn.expand '%:t', vim.fn.expand '%:e'
    file_extension = file_extension ~= '' and file_extension or vim.bo.filetype
    icon, icon_highlight_group = devicons.get_icon(file_name, file_extension)
    ...
    if highlight_color then
        ...
        if not icon_highlight or not modules.highlight.highlight_exists(icon_highlight.name .. 'normal') then
...
function M:get_current_filename_with_icon()
    ...
    local file_name = get_current_filename()
