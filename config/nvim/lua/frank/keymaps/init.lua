...
function M.init()
    ...
    -- Tree
    keymap(n_v, 'gc', function() require('frank.keymaps.utils').cwd_current_buffer() end, default_settings)
    keymap(n_v, '<Leader>t', function() require('frank.keymaps.utils').toggle_tree() end, default_settings)

    -- Barbar
    keymap(n, '<A-,>', '<Cmd>BufferPrevious<CR>', default_settings)
    keymap(n, '<A-.>', '<Cmd>BufferNext<CR>', default_settings)
    keymap(n, '<A-c>', '<Cmd>BufferClose<CR>', default_settings)
...
function M.editing()
    ...
    keymap(ex_t, '<C-s>', function() require('frank.keymaps.utils').save_file() end, default_settings)
    ...
    keymap(n, '<leader>v', function() require('frank.keymaps.utils').toggle_diffview() end)
...
function M.lspsaga()
    ...
    keymap(n, '[e', function() require('frank.keymaps.utils').prev_diag() end, default_settings)
    keymap(n, ']e', function() require('frank.keymaps.utils').next_diag() end, default_settings)
    keymap(n, '[E', function() require('frank.keymaps.utils').prev_error() end, default_settings)
    keymap(n, ']E', function() require('frank.keymaps.utils').next_error() end, default_settings)
    keymap(n, 'gr', '<Cmd>Telescope lsp_references<CR>', default_settings)
end

function M.debugger()
    keymap(n, '<C-b>', '<Cmd>DapToggleBreakpoint<CR>', default_settings)
    keymap(n, '<leader>s', function() require('frank.keymaps.utils').dap_float_scope() end, default_settings)
    keymap(n, '<F1>', function() require('frank.keymaps.utils').dap_toggle_ui() end, default_settings)
